import random, os, shapefile, numpy

print("Initializing")

if(os.path.isfile("d.csv") and os.path.isfile("prev.txt")):
    delete = input("Previous data has been found. Would you like to delete and regenerate new data?(y/n) ").lower()
    while(not delete == "y" and not delete == "n"):
...
if(numpy.logical_xor(os.path.isfile("d.csv"), os.path.isfile("prev.txt"))):
    delete = input("Some data is missing. This could lead to repition of data. Would you like to delete and regenerate new data?(y/n) ").lower()
    while(not delete == "y" and not delete == "n"):
        print("Invalid response. Please try again.")
        delete = input("Some data is missing. This could lead to repetition of data. Would you like to regenerate the data?(y/n) ").lower()
        
    if(delete == "y"):
        if(os.path.isfile("d.csv")):
            os.remove("d.csv")
        elif(os.path.isfile("prev.txt")):
            os.remove("prev.txt")
    ...
    for i in firstNames[iStart:]:
        for j in middleNames[jStart:]:
            for k in lastNames[kStart:]:
                ...
                lat = pos["lat"]
                long = pos["long"]
                text = str(idValue)+", "+i+", "+j+", "+k+", "+sex+", "+str(age)+", "+education+", "+str(income)+", "+str(networth)+", "+str(streetNum)+", "+street+", "+village+", "+mandal+", "+district+", "+state+", "+str(pin)+", "+str(lat)+", "+str(long)+"\n"
                
                d.seek(0)
                if(not any(name in s for s in d.readlines())):
                    dList.append(text)
                numOfPeopleAdded += 1
